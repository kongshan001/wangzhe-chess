...
class SecuritySettings(BaseSettings):
    ...
    REFRESH_TOKEN_EXPIRE_DAYS: int = Field(
        default=7, ge=1, le=30, description="刷新令牌过期时间（天）"
    )
    ...
    RATE_LIMIT_PER_MINUTE: int = Field(
        default=60, ge=0, description="每分钟请求限制（0 表示不限制）"
    )
...
class GameSettings(BaseSettings):
    ...
    MAX_PLAYERS_PER_ROOM: int = Field(default=8, ge=2, le=16, description="每个房间最大玩家数")
    TICK_RATE: int = Field(default=30, ge=10, le=60, description="游戏帧率（FPS）")
    ROUND_DURATION_SECONDS: int = Field(default=30, ge=10, le=120, description="回合持续时间（秒）")
    BUY_PHASE_DURATION: int = Field(default=30, ge=15, le=60, description="购买阶段持续时间（秒）")
    BATTLE_PHASE_DURATION: int = Field(
        default=60, ge=30, le=120, description="战斗阶段持续时间（秒）"
    )
