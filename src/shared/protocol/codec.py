...
class MessageCodecError(Exception):
    ...
    def __init__(self, message: str, code: int = 0, details: Optional[dict[str, Any]] = None) -> None:
        """
        初始化错误
...
class MessageEncoder:
    ...
    def __init__(self, *, indent: Optional[int] = None, ensure_ascii: bool = False) -> None:
        """
        初始化编码器
    ...
    def decode(self, data: str | bytes | dict[str, Any]) -> BaseMessage:
        ...
        try:
            ...
            raise MessageCodecError(
                f"JSON 解析失败: {e}",
                code=1007,
                details={"position": e.pos, "message": e.msg}
            ) from e
        except Exception as e:
            raise MessageCodecError(
                f"解码消息失败: {e}",
...
def get_message_class(message_type: MessageType) -> Optional[type[BaseMessage]]:
    """
    获取消息类型对应的类
