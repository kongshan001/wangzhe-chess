...
class TutorialManager:
    ...
    def get_tutorials_for_player(
        ...
        for tutorial in sorted(
            self.tutorials.values(), key=lambda t: (t.sort_order, t.recommended_level)
        ):
    ...
    def update_progress(
        ...
        if progress.current_step > tutorial.total_steps:
            progress.complete()
            logger.info(f"Player {player_id} completed tutorial: {tutorial.name}")
    ...
    def complete_tutorial(
        ...
        logger.info(f"Player {player_id} manually completed tutorial: {tutorial.name}")
    ...
    def skip_tutorial(
        ...
        logger.info(f"Player {player_id} skipped tutorial: {tutorial.name}")
    ...
    def claim_reward(
        ...
        logger.info(f"Player {player_id} claimed reward for tutorial: {tutorial.name}")
    ...
    def reset_progress(
        ...
        logger.info(f"Player {player_id} reset progress for tutorial: {tutorial_id}")
...
def get_tutorial_manager(config_path: str | None = None) -> TutorialManager:
    """
    获取引导管理器单例
