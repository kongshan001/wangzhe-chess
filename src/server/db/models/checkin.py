...
class CheckinStreakDB(Base, IdMixin, TimestampMixin):
    ...
    cycle_start_date: Mapped[Optional[date]] = mapped_column(
        Date,
        nullable=True,
        comment="当前周期开始日期",
    )
    
    last_reset_month: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
    ...
    def to_dict(self) -> dict:
        """转换为字典"""
        data = super().to_dict()
        data["last_checkin_date"] = self.last_checkin_date.isoformat() if self.last_checkin_date else None
        data["cycle_start_date"] = self.cycle_start_date.isoformat() if self.cycle_start_date else None
        return data
