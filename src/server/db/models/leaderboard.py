...
class LeaderboardDB(Base, IdMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "leaderboard_type", "period", "period_start", name="uq_leaderboard_type_period"
        ),
...
class LeaderboardRewardDB(Base, IdMixin, TimestampMixin):
    ...
    avatar_frame: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
        comment="头像框ID",
    )
...
class PlayerLeaderboardStatsDB(Base, IdMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "player_id", "leaderboard_type", "period", "period_start", name="uq_player_leaderboard"
        ),
    ...
    reward_claimed_at: Mapped[datetime | None] = mapped_column(
        DateTime,
        nullable=True,
        comment="领取奖励时间",
    )
