...
class VotingPollDB(Base, IdMixin, TimestampMixin):
    ...
    winning_option_id: Mapped[Optional[str]] = mapped_column(
        String(64),
        nullable=True,
        comment="获胜选项ID",
    )
    ...
    participation_reward_config: Mapped[Optional[dict]] = mapped_column(
        JSON,
        nullable=True,
        comment="参与奖励配置",
    )
    
    win_bonus_reward_config: Mapped[Optional[dict]] = mapped_column(
        JSON,
        nullable=True,
        comment="投中额外奖励配置",
    )
...
class PlayerVoteDB(Base, IdMixin, TimestampMixin):
    ...
    rewards_claimed_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime,
        nullable=True,
        comment="奖励领取时间",
    )
    ...
    def to_dict(self) -> dict:
        ...
        data["rewards_claimed_at"] = self.rewards_claimed_at.isoformat() if self.rewards_claimed_at else None
        return data
