...
class CustomRoomDB(Base, IdMixin, TimestampMixin):
    ...
    special_rules: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="特殊规则",
    )
    ...
    game_ended_at: Mapped[datetime | None] = mapped_column(
        DateTime,
        nullable=True,
        comment="游戏结束时间",
    )
...
class CustomRoomPlayerDB(Base, IdMixin, TimestampMixin):
    ...
    final_hp: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="最终血量",
    )
...
class CustomRoomStatsDB(Base, IdMixin, TimestampMixin):
    ...
    favorite_rules: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="最喜欢的规则",
    )
