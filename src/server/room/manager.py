...
from .game_room import GameRoom, RoomState
...
class RoomManager:
    ...
    def __init__(self) -> None:
        ...
        # 回调函数
        self._on_room_created: Callable[[GameRoom], None] | None = None
        self._on_room_destroyed: Callable[[str], None] | None = None
        self._on_player_join_room: Callable[[int, str], None] | None = None
        self._on_player_leave_room: Callable[[int, str], None] | None = None
    ...
    async def get_player_room(self, player_id: int) -> GameRoom | None:
        """
        获取玩家所在的房间
    ...
    async def find_available_room(self) -> GameRoom | None:
        """
        查找一个可加入的房间
    ...
    def __repr__(self) -> str:
        return f"<RoomManager(rooms={len(self.rooms)}, players={len(self.player_rooms)})>"
