...
class SkinManager:
    ...
    def get_player_skins_by_hero(self, player_id: str, hero_id: str) -> List[PlayerSkin]:
        ...
        player_skin_dict = self.player_skins.get(player_id, {})
        return [
            ps for ps in player_skin_dict.values()
            if ps.hero_id == hero_id
        ]
    ...
    def get_equipped_skin(self, player_id: str, hero_id: str) -> Optional[str]:
        """
        获取玩家装备的皮肤
    ...
    def get_equipped_skin_config(self, player_id: str, hero_id: str) -> Optional[Skin]:
        """
        获取玩家装备的皮肤配置
    ...
    ) -> Tuple[Optional[PlayerSkin], str, int, Optional[str]]:
        """
        购买皮肤
    ...
    ) -> Tuple[Optional[PlayerSkin], Optional[str]]:
        """
        解锁皮肤（奖励、成就、活动等）
...
def get_skin_manager(config_path: Optional[str] = None) -> SkinManager:
    """
    获取皮肤管理器单例
