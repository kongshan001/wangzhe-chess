...
class WebSocketBatchProcessor:
    ...
    async def enqueue(
        ...
        async with self._lock:
            ...
            if client_id not in self._batch_tasks or self._batch_tasks[client_id].done():
                self._batch_tasks[client_id] = asyncio.create_task(self._process_batch(client_id))
    ...
    async def flush(self, client_id: str | None = None) -> None:
        """
        立即刷新队列
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            **self._stats,
            "queue_sizes": {client_id: len(queue) for client_id, queue in self._queues.items()},
