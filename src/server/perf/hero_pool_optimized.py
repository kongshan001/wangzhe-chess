...
class OptimizedSharedHeroPool:
    ...
    def __init__(self, config_loader: HeroConfigLoader, seed: int | None = None) -> None:
        ...
        # 预计算每个费用的总可用数量
        self._cost_totals: dict[int, int] = dict.fromkeys(
            range(MIN_HERO_COST, MAX_HERO_COST + 1), 0
        )
    ...
    def draw_random_hero_batch(self, cost: int, count: int) -> list[HeroTemplate]:
        """
        批量随机抽取指定费用的英雄
        ...
        if HAS_NUMPY and count > 5:
            ...
            indices = np.random.choice(
                len(hero_ids), size=sample_count, replace=False, p=weights_arr
            )
...
class OptimizedShopManager:
    ...
    def buy_hero(self, slot_index: int) -> HeroTemplate | None:
        """购买英雄"""
