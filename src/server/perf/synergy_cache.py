...
from shared.models import Hero, ActiveSynergy, SynergyType
    ...
    def update(
        ...
        if added_heroes:
            for hero in added_heroes:
                ...
                if hero.profession:
                    self._current_counts[hero.profession] = self._current_counts.get(hero.profession, 0) + 1
        ...
        if removed_heroes:
            for hero in removed_heroes:
                ...
                if hero.profession and hero.profession in self._current_counts:
                    self._current_counts[hero.profession] = max(0, self._current_counts[hero.profession] - 1)
        
        if changed_heroes:
            for old_hero, new_hero in changed_heroes:
                # 移除旧状态
                if old_hero.race and old_hero.race in self._current_counts:
                    self._current_counts[old_hero.race] = max(0, self._current_counts[old_hero.race] - 1)
                if old_hero.profession and old_hero.profession in self._current_counts:
                    self._current_counts[old_hero.profession] = max(0, self._current_counts[old_hero.profession] - 1)
                
                # 添加新状态
                if new_hero.race:
                    self._current_counts[new_hero.race] = self._current_counts.get(new_hero.race, 0) + 1
                if new_hero.profession:
                    self._current_counts[new_hero.profession] = self._current_counts.get(new_hero.profession, 0) + 1
            ...
            if count > 0:
                ...
                if synergy:
                    ...
                    if active_level or count > 0:
                        self._current_synergies.append(ActiveSynergy(
                            synergy_name=race,
