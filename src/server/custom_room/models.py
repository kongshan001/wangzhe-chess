...
class RoomSettings:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RoomSettings":
        """从字典创建"""
        special_rules = [
            SpecialRuleType(r) for r in data.get("special_rules", [])
        ]
        return cls(
...
class CustomRoom:
    ...
    game_ended_at: Optional[datetime] = None
    ...
    def can_start(self) -> bool:
        ...
        return (
            ...
            and self.state == CustomRoomState.WAITING
        )
    ...
    def get_player_by_slot(self, slot: int) -> Optional[RoomPlayer]:
        """根据位置获取玩家"""
    ...
    def fill_with_ai(self, count: Optional[int] = None) -> List[RoomPlayer]:
        """
        用 AI 填充空位
    ...
    def to_summary(self) -> Dict[str, Any]:
        ...
        return {
            "room_id": self.room_id,
            "name": self.name,
            "host_name": self.players[self.host_id].nickname if self.host_id and self.host_id in self.players else "未知",
            "player_count": self.player_count,
