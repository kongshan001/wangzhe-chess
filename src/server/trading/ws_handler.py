...
class TradingWSHandler:
    ...
    def __init__(self, trading_manager: TradingManager | None = None):
        """
        初始化交易 WebSocket 处理器
    ...
    async def handle_confirm_trade(
        ...
        return TradeConfirmedMessage(
            ...
            receiver_confirmed=trade_request.receiver_offer.confirmed
            if trade_request.receiver_offer
            else False,
            both_confirmed=trade_request.is_both_confirmed(),
            status=trade_request.status.value,
            seq=message.seq,
    ...
    async def handle_get_trade_status(
        ...
        return TradeStatusMessage(
            ...
            receiver_items=[item.to_dict() for item in trade.receiver_offer.items]
...
class SendTradeRequestMessage(BaseMessage):
    ...
    message: str | None = Field(default=None, description="附带消息")
...
class GetTradeHistoryMessage(BaseMessage):
    ...
    limit: int | None = Field(default=20, description="数量限制")
    offset: int | None = Field(default=0, description="偏移量")
