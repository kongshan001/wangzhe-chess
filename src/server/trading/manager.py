...
class TradingManager:
    ...
    def create_trade_request(
    ...
    ) -> tuple[TradeRequest | None, str | None]:
        """
        创建交易请求
        ...
        logger.info(f"Trade request created: {trade_id} from {sender_id} to {receiver_id}")
    ...
    def accept_trade(
        ...
        logger.info(f"Trade accepted: {trade_id} by {receiver_id}")
    ...
    def reject_trade(
        ...
        logger.info(f"Trade rejected: {trade_id} by {player_id}, reason: {reason}")
    ...
    def cancel_trade(
        ...
        logger.info(f"Trade cancelled: {trade_id} by {player_id}")
    ...
    def confirm_trade(
        ...
        logger.info(f"Trade confirmed: {trade_id} by {player_id}")
    ...
    def execute_trade(
        ...
        self._transfer_items(
            trade.sender_id, trade.receiver_id or "", sender_received, receiver_received
        )
        ...
        logger.info(f"Trade executed: {trade_id}, tax: {total_tax}")
    ...
    def get_trade(self, trade_id: str) -> TradeRequest | None:
        """
        获取交易请求
    ...
    def can_player_trade(self, player_id: str) -> tuple[bool, str | None]:
        """
        检查玩家是否可以交易
    ...
    def _transfer_items(
        ...
        """
        # 更新发起方库存
        sender_inv = self.player_inventories.get(
            sender_id,
            {
                "shards": {},
                "equipment": {},
        ...
        # 更新接收方库存
        if receiver_id:
            receiver_inv = self.player_inventories.get(
                receiver_id,
                {
                    "shards": {},
                    "equipment": {},
    ...
    def clear_cache(self, player_id: str | None = None) -> None:
        ...
        if player_id:
            self.pending_trades = {
                k: v
                for k, v in self.pending_trades.items()
