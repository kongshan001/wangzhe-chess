...
class HeroShardManager:
    ...
    def can_compose(
        ...
        if shard_quantity < config.shards_required:
            return False, f"碎片不足，需要 {config.shards_required} 碎片，当前只有 {shard_quantity} 碎片"
        
        # 检查需要的英雄数量（2星及以上需要）
        if config.same_star_heroes > 0:
            player_hero_list = self.player_heroes.get(player_id, [])
            same_star_count = sum(
                1 for h in player_hero_list
                if h.get("hero_id") == hero_id and h.get("star", 1) == config.hero_star_required
            )
            
            if same_star_count < config.same_star_heroes:
                return False, f"英雄数量不足，需要 {config.same_star_heroes} 个 {config.hero_star_required} 星 {hero_name}，当前只有 {same_star_count} 个"
    ...
    def compose_hero(
        ...
        heroes_used = 0
        if config.same_star_heroes > 0:
            self._consume_heroes(player_id, hero_id, config.hero_star_required, config.same_star_heroes)
...
# 全局单例
_hero_shard_manager: Optional[HeroShardManager] = None
