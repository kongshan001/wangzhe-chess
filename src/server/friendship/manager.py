...
class FriendshipManager:
    ...
    def get_friend(self, player_id: str, friend_id: str) -> FriendInfo | None:
        """
        获取单个好友信息
    ...
    def send_friend_request(
        ...
        logger.info(f"Friend request sent: {from_player_id} -> {to_player_id} ({request_id})")
    ...
    def reject_friend_request(self, request_id: str) -> bool:
        ...
        logger.info(f"Friend request rejected: {request.from_player_id} -> {request.to_player_id}")
    ...
    def send_private_message(
        ...
        if len(self.private_messages[chat_key]) > self._max_messages_per_chat:
            self.private_messages[chat_key] = self.private_messages[chat_key][
                -self._max_messages_per_chat :
            ]
    ...
    def get_player_team(self, player_id: str) -> TeamInfo | None:
        """获取玩家所在队伍"""
...
# 全局单例
_friendship_manager: FriendshipManager | None = None
