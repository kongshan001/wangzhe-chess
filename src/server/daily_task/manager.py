...
class DailyTaskManager:
    ...
    def __init__(
        ...
        self.player_tasks: dict[
            str, dict[str, DailyTask]
        ] = {}  # player_id -> {task_id -> DailyTask}
        self.player_progress: dict[
            str, TaskProgress
        ] = {}  # f"{player_id}_{task_id}" -> TaskProgress
        ...
        default_templates = [
            ...
            TaskTemplate(
                ...
                requirement=TaskRequirement(
                    type=TaskType.TOP_FINISH, target=3, conditions={"rank": 4}
                ),
                rewards=TaskReward(gold=150, exp=80),
                difficulty=TaskDifficulty.EASY,
                weight=85,
    ...
    def _select_random_templates(
        self, count: int, exclude_ids: list[str] = None
    ) -> list[TaskTemplate]:
        """
        根据权重随机选择任务模板
        ...
        # 筛选可用模板
        available = [t for t in self.templates.values() if t.template_id not in exclude_ids]
    ...
    def get_task(self, player_id: str, task_id: str) -> DailyTask | None:
        """
        获取指定任务
    ...
    def get_task_progress(self, player_id: str, task_id: str) -> TaskProgress | None:
        """
        获取任务进度
    ...
    def update_progress(
        ...
        **kwargs,
        ...
        if just_completed:
            logger.info(f"Player {player_id} completed daily task: {task.name}")
    ...
    def add_progress(
        ...
        **kwargs,
        ...
        return self.update_progress(player_id, task_id, progress.progress + delta, **kwargs)
        ...
        **kwargs,
        ...
        for task in tasks.values():
            if task.requirement.type == task_type:
                progress = self.update_progress(player_id, task.task_id, value, **kwargs)
                if progress:
    ...
    def add_progress_by_type(
        ...
        **kwargs,
        ...
        for task in tasks.values():
            if task.requirement.type == task_type:
                progress = self.add_progress(player_id, task.task_id, delta, **kwargs)
                if progress:
                    updated.append((task, progress))
    ...
    def claim_reward(
        ...
        if progress.claim_reward():
            logger.info(f"Player {player_id} claimed reward for daily task: {task.name}")
            return task.rewards
    ...
    def get_full_task_info(self, player_id: str) -> list[dict[str, Any]]:
        ...
        for task in tasks:
            ...
            task_data["progress_percent"] = (
                min(100, (progress.progress / task.requirement.target) * 100) if progress else 0
            )
            task_data["refresh_cost"] = (
                0
                if self.get_remaining_free_refresh(player_id) > 0
...
def get_daily_task_manager(config_path: str | None = None) -> DailyTaskManager:
    """
    获取每日任务管理器单例
