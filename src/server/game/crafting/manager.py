...
class CraftingManager:
    ...
    def get_recipe(self, recipe_id: str) -> CraftingRecipe | None:
        """获取配方"""
    ...
    def can_craft(self, recipe: CraftingRecipe, inventory: PlayerInventory) -> tuple[bool, str]:
        """
        检查是否可以执行合成
    ...
    def craft(self, recipe: CraftingRecipe, inventory: PlayerInventory) -> CraftingResult:
        """
        执行装备合成
        ...
        if not can_craft:
            return CraftingResult(success=False, error_message=error_msg)
    ...
    def craft_with_equipment_ids(
        self, equipment_ids: list[str], inventory: PlayerInventory
    ) -> CraftingResult:
        """
        使用装备ID列表进行合成（自动匹配配方）
        ...
        if not matching:
            return CraftingResult(success=False, error_message="没有找到匹配的合成配方")
        ...
        for eq_id, needed in counts.items():
            if not inventory.has_equipment(eq_id, needed):
                return CraftingResult(success=False, error_message=f"缺少装备: {eq_id}")
...
def create_crafting_manager(config_path: str | None = None) -> CraftingManager:
    """
    创建合成管理器的工厂函数
    ...
    if config_path is None:
        # 尝试使用默认配置路径
        default_path = (
            Path(__file__).parent.parent.parent.parent.parent / "config" / "crafting-recipes.json"
        )
