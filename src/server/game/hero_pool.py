...
class HeroConfigLoader:
    ...
    def __init__(self) -> None:
        ...
        self.templates_by_cost: dict[int, list[HeroTemplate]] = {i: [] for i in range(MIN_HERO_COST, MAX_HERO_COST + 1)}
    ...
    def get_template(self, hero_id: str) -> Optional[HeroTemplate]:
        """
        获取英雄模板
...
class ShopManager:
    ...
    def buy_hero(self, slot_index: int, cost: int) -> Optional[HeroTemplate]:
        """
        购买商店槽位中的英雄
...
class HeroFactory:
    ...
    def create_hero(self, template_id: str, star: int = 1) -> Optional[Hero]:
        """
        创建英雄实例
...
def create_default_hero_pool(seed: Optional[int] = None) -> tuple[HeroConfigLoader, SharedHeroPool, HeroFactory]:
    """
    创建默认的英雄池系统
