...
class EmoteManager:
    ...
    def __init__(self, ws_handler: WebSocketHandler | None = None):
        """
        初始化表情管理器
    ...
    def get_emotes_by_category(self, category: EmoteCategory) -> list[Emote]:
        ...
        Returns:
            该分类的表情列表
        """
        return [emote for emote in self.emotes.values() if emote.category == category]

    def get_emote(self, emote_id: str) -> Emote | None:
        """
        获取单个表情
    ...
    def get_player_emote(self, player_id: str, emote_id: str) -> PlayerEmote | None:
        """
        获取玩家对特定表情的拥有状态
    ...
    def check_and_unlock_emotes(
        ...
        if newly_unlocked:
            logger.info(f"Player {player_id} unlocked {len(newly_unlocked)} new emotes")
    ...
    async def send_emote(
        ...
        if not self.has_emote(from_player_id, emote_id) and not emote.is_free:
            logger.warning(f"Player {from_player_id} doesn't have emote {emote_id}")
            return None

        # 检查冷却时间
        if not self.can_send_emote(from_player_id):
            remaining = self.get_cooldown_remaining(from_player_id)
            logger.debug(f"Player {from_player_id} on cooldown ({remaining:.1f}s remaining)")
            return None
    ...
    def set_emote_hotkey(
        ...
        if not self.has_emote(player_id, emote_id):
            ...
            if not emote.is_free:
                logger.warning(f"Player {player_id} doesn't have emote {emote_id}")
                return False
        ...
        logger.info(f"Player {player_id} set hotkey {hotkey} for emote {emote_id}")
    ...
    def remove_emote_hotkey(
        ...
        logger.info(f"Player {player_id} removed hotkey for emote {emote_id}")
    ...
    def get_emote_by_hotkey(self, player_id: str, hotkey: str) -> Emote | None:
        """
        根据快捷键获取表情
...
# 全局单例
_emote_manager: EmoteManager | None = None
