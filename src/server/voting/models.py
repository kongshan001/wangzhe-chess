...
class PlayerVote:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rewards_claimed_at": self.rewards_claimed_at.isoformat() if self.rewards_claimed_at else None,
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlayerVote":
        ...
        return cls(
            ...
            vote_time=datetime.fromisoformat(data["vote_time"]) if data.get("vote_time") else datetime.now(),
            is_vip=data.get("is_vip", False),
            rewards_claimed=data.get("rewards_claimed", False),
            rewards_claimed_at=datetime.fromisoformat(data["rewards_claimed_at"]) if data.get("rewards_claimed_at") else None,
        )
...
class VotingPoll:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "title": self.title,
            "description": self.description,
            "voting_type": self.voting_type.value if isinstance(self.voting_type, VotingType) else self.voting_type,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "VotingPoll":
        ...
        return cls(
            ...
            title=data["title"],
            description=data.get("description", ""),
            voting_type=VotingType(data["voting_type"]) if data.get("voting_type") else VotingType.CUSTOM,
            ...
            participation_reward=[VotingReward.from_dict(r) for r in data.get("participation_reward", [])],
    ...
    def get_option(self, option_id: str) -> Optional[VotingOption]:
        """获取指定选项"""
    ...
    def get_winning_option(self) -> Optional[VotingOption]:
        """获取获胜选项"""
        if not self.options:
            return None
...
class VotingResult:
    ...
    poll_id: str
    winning_option: Optional[VotingOption] = None
    total_votes: int = 0
    total_voters: int = 0
...
class VotingInfo:
    ...
    player_vote: Optional[PlayerVote] = None
    can_vote: bool = True
    reason: str = ""
