...
class VotingManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        logger.info(f"VotingManager initialized")
    ...
    def _create_poll_from_template(self, template: Dict[str, Any]) -> Optional[VotingPoll]:
        """从模板创建投票"""
        try:
            ...
            for r in template.get("participation_reward", []):
                participation_reward.append(VotingReward(
            ...
            for r in template.get("win_bonus_reward", []):
                win_bonus_reward.append(VotingReward(
                    reward_id=r.get("reward_id", f"reward_{uuid.uuid4().hex[:8]}"),
                    reward_type=RewardType(r["reward_type"]),
                    item_id=r.get("item_id"),
                    quantity=r.get("quantity", 1),
            ...
            now = datetime.now()
            start_time = now
            end_time = now + timedelta(days=template.get("duration_days", self.DEFAULT_DURATION_DAYS))
    ...
    def create_poll(
        ...
        participation_reward: Optional[List[VotingReward]] = None,
        win_bonus_reward: Optional[List[VotingReward]] = None,
        min_vip_level: int = 0,
    ...
    def get_poll(self, poll_id: str) -> Optional[VotingPoll]:
        """
        获取投票详情
    ...
    def get_polls(
        self,
        status: Optional[VotingStatus] = None,
    ...
    def get_player_vote(self, player_id: str, poll_id: str) -> Optional[PlayerVote]:
        """
        获取玩家投票记录
    ...
    def vote(
        ...
        option_id: str,
        vip_level: int = 0,
    ) -> Tuple[Optional[PlayerVote], Optional[str]]:
        """
        执行投票
    ...
    def end_poll(self, poll_id: str) -> Tuple[Optional[VotingResult], Optional[str]]:
        """
        结束投票
    ...
    def calculate_results(self, poll_id: str) -> Optional[VotingResult]:
        """
        计算投票结果
    ...
    def get_voting_info(self, player_id: str, poll_id: str) -> Optional[VotingInfo]:
        """
        获取投票信息（包含玩家投票状态）
    ...
    def get_rewards_to_claim(
        ...
        player_id: str,
        poll_id: str,
    ) -> Tuple[List[VotingReward], Optional[str]]:
        """
        获取待领取的奖励
    ...
    def cancel_poll(self, poll_id: str) -> Tuple[bool, Optional[str]]:
        """
        取消投票
...
def get_voting_manager(config_path: Optional[str] = None) -> VotingManager:
    """
    获取投票管理器单例
