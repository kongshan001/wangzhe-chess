...
from .models import PlayerSeasonData, Season, SeasonReward, SeasonStatus, Tier
...
class SeasonManager:
    ...
    def create_season(
        ...
        description: str = "",
        rewards: dict[int, SeasonReward] | None = None,
    ) -> Season:
        """
        创建新赛季
    ...
    def soft_reset_tier(self, current_tier: Tier) -> Tier:
        ...
        logger.info(f"Soft reset tier: {current_tier.display_name} -> {new_tier.display_name}")
    ...
    def add_pass_exp(
        ...
        if new_level > old_level:
            logger.info(f"Player {player_id} pass level up: {old_level} -> {new_level}")
    ...
    def get_season_ranking(
        ...
        # 筛选该赛季的玩家数据
        season_players = [
            data for key, data in self.player_data.items() if data.season_id == season_id
        ]

        # 按段位和前四率排序
        season_players.sort(key=lambda x: (x.final_tier.value, x.top4_rate), reverse=True)
...
# 全局单例
_season_manager: SeasonManager | None = None
