...
class ReplayManager:
    ...
    async def load_replay(self, replay_id: str) -> Replay | None:
        """
        加载回放
    ...
    def get_play_session(self, session_id: str) -> ReplaySession | None:
        """
        获取播放会话
        ...
        try:
            ...
            if success:
                ...
                share_codes_to_remove = [
                    code for code, rid in self._share_codes.items() if rid == replay_id
                ]
    ...
    async def _delete_oldest_replay(self, player_id: int) -> bool:
        ...
        try:
            ...
            async with self.database.get_session() as session:
                ...
                stmt = (
                    select(ReplayDB)
                    .where(ReplayDB.player_id == player_id)
                    .order_by(ReplayDB.created_at.asc())
                    .limit(1)
                )
                ...
                if oldest:
                    await session.execute(delete(ReplayDB).where(ReplayDB.id == oldest.id))
        ...
        try:
            ...
            async with self.database.get_session() as session:
                db_replay = ReplayDB(
                    ...
                    initial_state=json.dumps(replay.initial_state)
                    if replay.initial_state
                    else None,
                    final_rankings=json.dumps(replay.final_rankings)
                    if replay.final_rankings
                    else None,
        ...
        try:
            ...
            async with self.database.get_session() as session:
                ...
                stmt = (
                    select(ReplayDB)
                    .where(ReplayDB.player_id == player_id)
                    .order_by(ReplayDB.created_at.desc())
                    .offset(offset)
                    .limit(page_size)
                )
    ...
    async def _load_replay_from_db(self, replay_id: str) -> Replay | None:
        """从数据库加载回放"""
        try:
            from ..db.models.replay import ReplayDB
            ...
            async with self.database.get_session() as session:
                ...
                metadata = ReplayMetadata(
                    ...
                    created_at=int(db_replay.created_at.timestamp() * 1000)
                    if db_replay.created_at
                    else 0,
    ...
    async def _update_replay_share_code(
        ...
        try:
            ...
            async with self.database.get_session() as session:
                ...
                stmt = (
                    update(ReplayDB)
                    .where(ReplayDB.replay_id == replay_id)
                    .values(
                        share_code=share_code,
                        is_shared=True,
                    )
    ...
    async def _find_replay_by_share_code(self, share_code: str) -> str | None:
        """通过分享码查找回放ID"""
        try:
            from ..db.models.replay import ReplayDB
            ...
            async with self.database.get_session() as session:
                ...
                stmt = select(ReplayDB.replay_id).where(ReplayDB.share_code == share_code)
                result = await session.scalar(stmt)
                if result:
