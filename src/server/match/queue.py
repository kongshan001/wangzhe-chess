...
from .rating import PlayerRating, Tier
...
class PlayerInfo:
    ...
    def to_queue_entry(self) -> QueueEntry:
        ...
        return QueueEntry(
            ...
            rating=PlayerRating(
                player_id=self.player_id, tier=Tier.BRONZE, stars=0, lp=0, wins=0, losses=0
            ),
...
class QueueEntry:
    ...
    match_callback: Callable[[dict[str, Any]], Awaitable[None]] | None = None
...
class MatchQueue:
    ...
    def __init__(self, config: QueueConfig | None = None):
        ...
        # 按段位分组的队列
        self._queues: dict[Tier, list[QueueEntry]] = {tier: [] for tier in Tier}
        ...
        # 匹配成功回调
        self._match_callback: Callable[[list[QueueEntry]], Awaitable[None]] | None = None

        # 后台任务
        self._running = False
        self._processor_task: asyncio.Task | None = None

    def set_match_callback(self, callback: Callable[[list[QueueEntry]], Awaitable[None]]) -> None:
        """
        设置匹配成功回调
    ...
    async def leave(self, player_id: str) -> QueueEntry | None:
        """
        玩家离开队列
    ...
    async def get_entry(self, player_id: str) -> QueueEntry | None:
        """
        获取玩家的队列条目
    ...
    async def get_queue_size(self, tier: Tier | None = None) -> int:
        """
        获取队列大小
    ...
    async def force_match(self, player_ids: list[str]) -> list[QueueEntry] | None:
        """
        强制匹配指定玩家
    ...
    def get_queue_stats(self) -> dict[str, Any]:
        ...
        # 计算平均等待时间
        wait_times = [e.get_wait_time_seconds() for e in self._all_entries.values()]
...
class AIPlayerGenerator:
    ...
    @classmethod
    def generate_ai_player(cls, target_elo: int, tier: Tier) -> dict[str, Any]:
        """
        生成一个 AI 玩家
    ...
    @classmethod
    def generate_ai_team(cls, target_elo: int, tier: Tier, count: int) -> list[dict[str, Any]]:
        """
        生成一组 AI 玩家
        ...
        Returns:
            AI 玩家列表
        """
        return [cls.generate_ai_player(target_elo, tier) for _ in range(count)]
