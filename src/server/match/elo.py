...
from .rating import PlayerRating, SeasonManager, Tier
...
class EloCalculator:
    ...
    def get_dynamic_k(
        self, player_elo: PlayerElo, player_rating: PlayerRating | None = None
    ) -> int:
        """
        获取动态 K 值
    ...
    def calculate_elo_change(
        ...
        player_rating: PlayerRating | None = None,
    ) -> int:
        """
        计算 ELO 变化值
        ...
        expected = self.calculate_expected_score(player_elo.current_elo, opponent_elo.current_elo)
    ...
    def update_elo(
        ...
        # 计算 ELO 变化
        change = self.calculate_elo_change(player_elo, opponent_elo, score, player_rating)
    ...
    def batch_update(
        self, results: list[tuple[PlayerElo, PlayerElo, float]]
    ) -> list[dict[str, Any]]:
        """
        批量更新 ELO
...
class EloBasedRatingUpdater:
    ...
    def __init__(self, calculator: EloCalculator | None = None):
        """
        初始化更新器
    ...
    def update_rating_from_elo(
        self, player_elo: PlayerElo, player_rating: PlayerRating, elo_change: int
    ) -> dict[str, Any]:
        """
        根据 ELO 变化更新段位
    ...
    def process_game_result(
        ...
        # 更新段位
        rating_result = self.update_rating_from_elo(player_elo, player_rating, elo_result["change"])
