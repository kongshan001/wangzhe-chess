...
class ConsumableManager:
    ...
    def get_consumable(self, consumable_id: str) -> Optional[ConsumableItem]:
        """
        获取道具配置
    ...
    def add_consumable(
    ...
    ) -> Tuple[Optional[PlayerConsumable], Optional[str]]:
        """
        添加道具给玩家
        ...
        logger.info(
            f"Player {player_id} obtained {quantity} {consumable_id} via {acquire_type}"
        )
    ...
    def use_consumable(
    ...
    ) -> Tuple[bool, Optional[ConsumableUsage], Optional[str]]:
        """
        使用道具
        ...
        logger.info(
            f"Player {player_id} used {quantity} {consumable_id} in {context}"
        )
    ...
    def buy_consumable(
    ...
    ) -> Tuple[Optional[PlayerConsumable], str, int, Optional[str]]:
        """
        购买道具
        ...
        logger.info(
            f"Player {player_id} bought {quantity} {consumable_id} with {cost} {currency}"
        )
    ...
    def apply_effect(
        ...
        context: str = "match",
        context_id: Optional[str] = None,
    ) -> Tuple[bool, Optional[ActiveConsumableEffect], Optional[str]]:
        """
        应用道具效果
        ...
        active_effect = ActiveConsumableEffect(
            ...
            remaining_rounds=effect_config.duration_value if effect_config.duration_type == "rounds" else -1,
            context=context,
        )
    ...
    def get_active_effects(
        self,
        player_id: str,
        effect_type: Optional[ConsumableEffect] = None,
    ...
    def consume_effect(
        ...
        if player_id in self.active_effects:
            self.active_effects[player_id] = [
                e for e in self.active_effects[player_id]
                if e != effect
            ]
    ...
    def load_player_data(
        ...
        effects_data: Optional[List[Dict[str, Any]]] = None,
    ) -> None:
        """
        加载玩家道具数据
    ...
    def get_player_data(self, player_id: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        effects_data = [
            e.to_dict() for e in self.active_effects.get(player_id, [])
        ]
...
# 全局单例
_consumable_manager: Optional[ConsumableManager] = None


def get_consumable_manager(config_path: Optional[str] = None) -> ConsumableManager:
    """
    获取道具管理器单例
