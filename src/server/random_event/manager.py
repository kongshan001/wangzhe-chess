...
class RandomEventManager:
    ...
    def __init__(self, config_path: str | None = None) -> None:
        ...
        # 事件回调
        self._on_event_triggered: Callable[[str, RandomEvent, int, dict], None] | None = None
        self._on_effect_applied: Callable[[str, EventEffect, dict], None] | None = None
    ...
    def execute_event(
        ...
        for effect in event.effects:
            effect_result = self._execute_effect(room_id, effect, players, round_number, context)
        ...
        for i, config in enumerate(rewards_config):
            ...
            if roll <= cumulative:
                ...
                if reward_type == "gold":
                    return {
                        "type": "gold",
                        "amount": random.randint(config.get("min", 3), config.get("max", 15)),
                    }
                elif reward_type == "exp":
                    return {
                        "type": "exp",
                        "amount": random.randint(config.get("min", 2), config.get("max", 8)),
                    }
...
_random_event_manager: RandomEventManager | None = None
