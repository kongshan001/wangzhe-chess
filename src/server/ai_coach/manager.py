...
class AICoachManager:
    ...
    def analyze_lineup(
        ...
        suggestions = self._generate_suggestions(
            player_id, game_state, strengths, weaknesses
        )
    ...
    def _generate_suggestions(
        ...
        for weakness in weaknesses:
            suggestion = self._create_suggestion_for_weakness(
                player_id, game_state, weakness
            )
            if suggestion:
    ...
    def _get_early_game_suggestions(
    ...
    def _get_late_game_suggestions(
        self, game_state: Dict[str, Any]
    ) -> List[AISuggestion]:
        """获取后期建议"""
        suggestions = []
    ...
    def predict_win_rate(
        ...
        weighted_score = sum(
            factors[key] * weights.get(key, 0.2)
            for key in factors
        )
    ...
    def _update_player_stats(
        ...
        for syn_name in match_item.final_synergies:
            stats.favorite_synergies[syn_name] = (
                stats.favorite_synergies.get(syn_name, 0) + 1
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_matches_recorded": sum(
                len(history) for history in self._match_history.values()
            ),
...
_ai_coach_manager: Optional[AICoachManager] = None
