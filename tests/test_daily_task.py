...
class TestDailyTaskManager:
    ...
    def test_update_progress(self, manager):
        ...
        progress = manager.update_progress("player1", task.task_id, task.requirement.target)
    ...
    def test_update_progress_by_type(self, manager):
        ...
        updated = manager.update_progress_by_type("player1", TaskType.PLAY_GAMES, 10)
    ...
    def test_add_progress_by_type(self, manager):
        ...
        # 再增加一些
        updated = manager.add_progress_by_type("player1", TaskType.PLAY_GAMES, 2)
    ...
    def test_claim_reward(self, manager):
        ...
        # 先完成任务
        manager.update_progress("player1", task.task_id, task.requirement.target)
    ...
    def test_refresh_task_free(self, manager):
        ...
        # 第一次刷新应该免费
        new_task, cost, error = manager.refresh_task("player1", old_task.task_id, player_gold=0)
    ...
    def test_refresh_task_with_cost(self, manager):
        ...
        # 第二次刷新需要花费金币
        new_task, cost, error = manager.refresh_task("player1", old_task.task_id, player_gold=1000)
    ...
    def test_refresh_task_not_exists(self, manager):
        """测试刷新不存在的任务"""
        new_task, cost, error = manager.refresh_task("player1", "nonexistent", player_gold=1000)
...
class TestEdgeCases:
    ...
    def test_update_completed_task(self, manager):
        ...
        # 再次更新进度
        progress = manager.update_progress("player1", task.task_id, task.requirement.target + 10)
    ...
    def test_excessive_progress(self, manager):
        ...
        # 完成超过目标的进度
        progress = manager.update_progress("player1", task.task_id, task.requirement.target * 100)
