...
from server.match.rating import Tier, PlayerRating
...
from server.spectator.models import GameVisibility
...
class BattleSimulatorStressTest:
    ...
    async def run_all(self) -> list[PerformanceMetrics]:
        ...
        print(f"  总耗时: {result1.total_time_ms:.2f}ms, 吞吐量: {result1.ops_per_second:.2f} ops/s")
        
        print("测试1000场并发战斗...")
        result2 = await self.test_1000_battles_concurrent()
        self.results.append(result2)
        print(f"  总耗时: {result2.total_time_ms:.2f}ms, 吞吐量: {result2.ops_per_second:.2f} ops/s")
...
class MatchQueueStressTest:
    ...
    async def test_match_processing(self, queue: MatchQueue, player_count: int) -> PerformanceMetrics:
        """测试匹配处理性能"""
        # 先加入玩家
...
class HeroPoolStressTest:
    ...
    def run_all(self) -> list[PerformanceMetrics]:
        ...
        self.results.append(result1)
        print(f"  平均耗时: {result1.avg_latency_ms:.4f}ms, 吞吐量: {result1.ops_per_second:.2f} ops/s")
        
        print("测试英雄抽取(5000次)...")
        result2 = self.test_hero_draw(5000)
        self.results.append(result2)
        print(f"  平均耗时: {result2.avg_latency_ms:.4f}ms, 吞吐量: {result2.ops_per_second:.2f} ops/s")
...
class SpectatorStressTest:
    ...
    async def test_create_sessions(self, game_count: int = 100, spectators_per_game: int = 10) -> PerformanceMetrics:
        """测试创建观战会话"""
        ...
        return self._calculate_metrics(
            f"创建观战会话({game_count}x{spectators_per_game})",
            latencies,
            total_time
        )
    
    async def test_state_updates(self, game_count: int = 100, updates: int = 50) -> PerformanceMetrics:
        """测试状态更新"""
        manager = SpectatorManager()
        ...
        return self._calculate_metrics(
            f"状态更新({game_count}x{updates})",
            latencies,
            total_time
        )
    
    async def test_chat_messages(self, game_count: int = 50, messages_per_game: int = 100) -> PerformanceMetrics:
        """测试弹幕系统"""
        manager = SpectatorManager()
        ...
        return self._calculate_metrics(
            f"弹幕消息({game_count}x{messages_per_game})",
            latencies,
            total_time
        )
    ...
    async def run_all(self) -> list[PerformanceMetrics]:
        ...
        self.results.append(result3)
        print(f"  总耗时: {result3.total_time_ms:.2f}ms, 吞吐量: {result3.ops_per_second:.2f} msgs/s")
